        ...
        const userPrompt = `Analise este documento e extraia os dados para preencher um template de contrato. Retorne JSON com a estrutura abaixo:
{
  ...
  // Router legado `supplierLinks` removido. Frontend nao chamava nenhuma rota dele
  // (verificado com grep em client/) e a tabela supplier_links nao e mais SSOT
  // (substituida por supplier_company_links). As funcoes db.* legadas
  // (getSupplierLinks, getLinksByTargetCompany, createSupplierLink etc.) foram
  // mantidas no db.ts apenas para compatibilidade com migracoes/scripts.
});
